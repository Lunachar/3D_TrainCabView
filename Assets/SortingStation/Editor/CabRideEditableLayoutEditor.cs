...
namespace SortingStation.EditorTools
{
    ...
    public sealed class CabRideEditableLayoutEditor : Editor
    {
        ...
        private static void RemoveLegacyControlBlocks(RectTransform root)
        {
            for (int i = root.childCount - 1; i >= 0; i--)
            {
                ...
                if (child.name == CabRideEditableLayout.CabFrameName ||
                    ...
                    child.name == CabRideEditableLayout.RadioPlayerName ||
                    child.name == CabRideEditableLayout.ThrottleSliderName ||
                    child.name == CabRideEditableLayout.BrakeLeverName ||
        ...
        {
            action = default;
            if (name == CabRideEditableLayout.RadioPlayerName) { action = CabControlAction.Radio; return true; }
            if (name == CabRideEditableLayout.ThrottleSliderName) { action = CabControlAction.Throttle; return true; }
            if (name == CabRideEditableLayout.BrakeLeverName) { action = CabControlAction.Brake; return true; }
