...
namespace SortingStation.Tests
{
    public sealed class CabAccessibilityAndAudioTests
    {
        ...
        [Test]
        public void AudioService_RadioVolumeToggleUsesSixtyAndNinetyPercentWithFortyPercentAttenuation()
        {
            ...
            audio.SetRadio(true);
            Assert.That(audio.MusicVolume, Is.EqualTo(0.36f).Within(0.0001f),
                "The 60% radio setting is additionally reduced by the requested 40% overall radio attenuation.");

            audio.ToggleRadioMusicVolume();
            Assert.That(audio.RadioMusicVolumeMultiplier, Is.EqualTo(0.9f).Within(0.0001f));
            Assert.That(audio.MusicVolume, Is.EqualTo(0.54f).Within(0.0001f),
                "The 90% setting must preserve the same 40% attenuation.");
        ...
        [Test]
        public void PointerActivationCanUseHitControlInsteadOfTheFallbackButtonAction()
        {
            GameObject go = new GameObject("Pointer-routed button", typeof(RectTransform), typeof(CanvasRenderer),
                typeof(Image), typeof(Outline), typeof(AccessibleButton));
            AccessibleButton button = go.GetComponent<AccessibleButton>();
            int fallbackActivations = 0;
            Vector2 routedPosition = Vector2.zero;
            button.Initialize(null, null, "Фары", Color.gray, Color.green, Color.yellow, 0.96f,
                Color.white, Color.black, () => fallbackActivations++);
            button.ConfigurePointerActivation(eventData => routedPosition = eventData.position);

            Vector2 pointer = new Vector2(321f, 654f);
            button.OnPointerClick(new PointerEventData(null) { position = pointer });

            Assert.That(routedPosition, Is.EqualTo(pointer));
            Assert.That(fallbackActivations, Is.Zero);
            Object.DestroyImmediate(go);
        }
        ...
        public void SceneryCatalog_HasGroundMountainsShadowsAndBothSleeperMaterials()
        {
            ...
            string[] shadowlessPoolObjects = catalog.Scenery
                .Where(entry => entry != null && entry.poolSpawn && entry.shadowMode == SceneryShadowMode.Off)
                .Select(entry => entry.id)
                .OrderBy(id => id)
                .ToArray();
            CollectionAssert.AreEquivalent(new[]
            {
                "real_animated_airplane", "real_animated_birds", "real_animated_boat", "real_animated_smoke_chimney"
            }, shadowlessPoolObjects,
                "Only airborne, waterborne, or particle-like effects should intentionally omit ground contact shadows.");
