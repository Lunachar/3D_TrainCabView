...
namespace SortingStation
{
    public sealed class MainMenuController : MonoBehaviour
    {
        ...
        private bool directSeasonSelection;
        ...
        private void Build()
        {
            ...
            int splashIndex = Random.Range(1, 6);
            Texture2D trainSplash = Resources.Load<Texture2D>("Cab3D/BootTrainSplash_v" + splashIndex);
            if (trainSplash != null)
            {
                GameObject splashObject = new GameObject("PhotorealTrainBackground", typeof(RectTransform), typeof(CanvasRenderer), typeof(RawImage));
                splashObject.transform.SetParent(root, false);
                RawImage splash = splashObject.GetComponent<RawImage>();
                splash.texture = trainSplash;
                splash.color = Color.white;
                splash.raycastTarget = false;
                UiFactory.Stretch(splash.rectTransform);
            }
            else
            {
        ...
        private void OpenSeasonSelection()
        {
            ...
            if (!directSeasonSelection) mainFocus.enabled = false;
            RectTransform overlay = UiFactory.Panel("SeasonOverlay", root, new Color(0f, 0f, 0f, directSeasonSelection ? 0.42f : 0.74f));
            ...
            if (directSeasonSelection) focus.Cancelled += OpenExitFromSeason;
            else focus.Cancelled += CloseModal;
            ...
            {
                AccessibleButton exit = UiFactory.Button("ExitGame", card, focus, "Выйти из игры", services.Settings.BrakeColor,
                    services.Settings.SelectedColor, OpenExitFromSeason, services.Settings.StatusFontSize);
                UiFactory.SetRect(exit.RectTransform, new Vector2(0.36f, 0.035f), new Vector2(0.64f, 0.15f), Vector2.zero, Vector2.zero);
            }
        }
        ...
        private void OpenExitFromSeason()
        {
