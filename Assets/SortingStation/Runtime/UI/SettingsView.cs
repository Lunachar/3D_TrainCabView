...
namespace SortingStation
{
    public sealed class SettingsView : MonoBehaviour
    {
        ...
        private void Build()
        {
            ...
            AccessibleButton worldMode = null;
            ...
            worldQuality = UiFactory.Button("CabWorldQuality", card, focusGroup, WorldQualityLabel(), theme.PanelAltColor,
                theme.SelectedColor, () =>
                {
                    services.Preferences.cabWorldQuality = services.Preferences.cabWorldQuality == CabWorldQuality.Balanced
                        ? CabWorldQuality.Performance : CabWorldQuality.Balanced;
                    worldQuality.SetLabel(WorldQualityLabel());
                    worldQuality.SetSelected(services.Preferences.cabWorldQuality == CabWorldQuality.Performance);
                    services.SavePreferences();
                }, 25);
            UiFactory.SetRect(worldQuality.RectTransform, new Vector2(0.51f, 0.755f), new Vector2(0.96f, 0.855f), Vector2.zero, Vector2.zero);
            worldQuality.SetSelected(services.Preferences.cabWorldQuality == CabWorldQuality.Performance);

            CreateVolumeRow(card, "Общая громкость", 0.63f, () => services.Preferences.masterVolume,
                value => services.Preferences.masterVolume = value);
            CreateVolumeRow(card, "Музыка", 0.51f, () => services.Preferences.musicVolume,
                value => services.Preferences.musicVolume = value);
            CreateVolumeRow(card, "Звуки", 0.39f, () => services.Preferences.effectsVolume,
