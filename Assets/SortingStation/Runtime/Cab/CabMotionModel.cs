...
namespace SortingStation
{
    public sealed class CabMotionModel
    {
        ...
        public void Step(float unscaledDeltaTime, float brake01, float externalTractionMultiplier)
        {
            ...
            // A sudden external traction cut (for example, an automatic station hold) can arrive
            // while SmoothDamp still carries positive acceleration from the preceding frames.
            // Clear only that forward momentum before easing toward the lower target so the train
            // coasts down smoothly instead of briefly accelerating after traction is removed.
            if (brake <= 0.001f && target < Speed01 && smoothingVelocity > 0f)
