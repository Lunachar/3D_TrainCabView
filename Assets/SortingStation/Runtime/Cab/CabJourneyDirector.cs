...
namespace SortingStation
{
    public sealed class CabJourneyDirector : MonoBehaviour
    {
        ...
        public void Initialize(ICabWorldView worldRenderer, CabRideDefinition rideDefinition,
        ...
        {
            // The hybrid view has physical signals and speed boards on the route.  The old
            // screen-space marker projection is useful in Legacy2D but looks like a floating sign
            // in a 3D landscape, so do not add a duplicate marker layer there.
            if (world is CabWorld3DRenderer) return;
            for (int i = 0; i < 5; i++)
            {
        ...
        private void UpdateSunGlare(Vector2 sunPosition, bool sunVisible)
        {
        ...
        public static bool ShouldShowUiSun(float weatherVisibility, bool isInsideTunnel)
        {
            return !isInsideTunnel && weatherVisibility > 0.001f;
        }
