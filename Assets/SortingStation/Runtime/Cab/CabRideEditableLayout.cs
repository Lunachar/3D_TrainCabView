...
namespace SortingStation
{
    ...
    public sealed class CabRideEditableLayout : MonoBehaviour
    {
        ...
        {
            if (action == CabControlAction.Radio) return RadioPlayerName;
            if (action == CabControlAction.Throttle) return ThrottleSliderName;
            if (action == CabControlAction.Brake) return BrakeLeverName;
