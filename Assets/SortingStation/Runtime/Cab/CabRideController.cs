...
namespace SortingStation
{
    public sealed class CabRideController : MonoBehaviour
    {
        ...
        private void Update()
        {
            // Unity can invoke Update once after a script hot-reload while the in-flight cab is
            // rebuilding. Do not turn a transient compile error into repeated console noise.
            if (services == null || motion == null || world == null) return;
            float dt = Mathf.Min(Time.unscaledDeltaTime, 0.05f);
            if (discardFirstMotionFrame)
            {
                discardFirstMotionFrame = false;
                dt = 0f;
            }
            ...
            bool tunnelAutoLight = isInsideTunnel;
            world.SetHeadlights(headlights || tunnelAutoLight);
            ...
            AnimateCabSway(dt);
            AnimateKeychain(dt);
            interior3D?.SetKeychainAngle(keychainAngle);
        ...
        {
            if (automaticStop) return "СТОП • ПРОВЕРКА";
            if (vigilanceAlarm) return "НАЖМИ КРАСНУЮ КНОПКУ";
            if (stationStop != null)
            {
                if (stationStop.Phase == CabStationPhase.DoorsOpen) return "ДВЕРИ ОТКРЫТЫ";
                if (stationStop.Phase == CabStationPhase.WaitingForDoors) return "ОТКРОЙ ДВЕРИ";
                if (stationStop.Phase == CabStationPhase.Approaching) return "ПОДХОД К СТАНЦИИ";
            }
            if (radio) return "РАДИО • В ЭФИРЕ";
            if (wipers) return "ДВОРНИКИ ВКЛ.";
            if (windowHeater) return "ОБОГРЕВ СТЕКЛА";
            return headlights ? "ФАРЫ ГОТОВЫ" : cabinLight ? "СВЕТ КАБИНЫ" : "СИСТЕМА ГОТОВА";
        }
        ...
        private void Build()
        {
            ...
            {
                CabWorldRenderer legacy = stage.gameObject.AddComponent<CabWorldRenderer>();
                legacy.Initialize(stage, services.Visuals.cabLandscape, services.CabRide, services.CabScenery, services.Preferences);
                world = legacy;
            }
            world.SegmentChanged += OnSegmentChanged;
            world.AmbientSoundRequested += OnAmbientSoundRequested;
            ...
            {
                // Retain only a faint source-photo rim. A stronger full-image blend ghosts the
                // old photographed dashboard through the new physical 3D controls.
                CabWorld3DSettings cockpitSettings = Resources.Load<CabWorld3DSettings>("Configuration/CabWorld3DSettings");
                float configuredOpacity = cockpitSettings != null ? cockpitSettings.CockpitPhotoOverlayOpacity : 0.12f;
                overlay.color = new Color(1f, 1f, 1f, HybridPhotoOverlayOpacity(configuredOpacity));
            }
            ...
            else if (services.Preferences.cabWorldMode == CabWorldMode.Hybrid3D)
            {
                ...
                interior3D.Initialize(cabInterior, services.Preferences.cabWorldQuality,
                    ActivateControl, AcknowledgeDispatcher);
            }
        ...
        private void BuildGentleInteractions()
        {
            ...
            passengerReport = new CabPassengerStopReport(null, 0, 0, passengers.OnboardCount, passengers.Capacity);
        ...
        private void OnStationPhaseChanged(CabStationPhase phase)
        {
            ...
            else if (phase == CabStationPhase.DoorsOpen)
            {
                passengerReport = passengers != null
                    ? passengers.CallAt(stationStop != null ? stationStop.CurrentStation : null)
                    : passengerReport;
                world?.SetPassengerReport(passengerReport);
                SetStatus(passengerReport != null ? passengerReport.StopText : "Двери открыты");
            }
            else if (phase == CabStationPhase.Releasing) SetStatus("Двери закрыты. Можно продолжать путь");
        }

        private void OnStationApproaching(CabStationDefinition station)
        {
        ...
        private void BuildHeadlightLayer()
        {
            ...
            UiFactory.SetRect(headlightGlow.rectTransform, new Vector2(0.32f, 0.465f), new Vector2(0.68f, 0.705f), Vector2.zero, Vector2.zero);
        ...
        private void BuildControls()
        {
            ...
            // The invisible part of every button remains the full tablet-friendly hit target.
            ...
            for (int i = 0; i < bindings.Length; i++)
            {
                ...
                {
                    idle = new Color(0.16f, 0.055f, 0.045f, usePhysicalCockpit ? Mathf.Max(quietOverlayAlpha, 0.34f) : 0.72f);
                ...
                if (usePhysicalCockpit)
                {
                    button.ConfigurePointerActivation(eventData =>
                    {
                        if (interior3D != null && interior3D.TryHitControl(eventData.position,
                                out CabControlAction physicalAction, out bool isDispatcherAcknowledgement))
                        {
                            if (isDispatcherAcknowledgement) AcknowledgeDispatcher();
                            else ActivateControl(physicalAction);
                        }
                        else ActivateControl(binding.action);
                    });
                }
        ...
        private void BuildDispatcherButton()
        {
            ...
            if (UsesPhysicalCockpit)
            {
                ...
                {
                    if (interior3D != null && interior3D.TryHitControl(eventData.position, out CabControlAction physicalAction,
                            out bool isDispatcherAcknowledgement))
                    {
                        if (isDispatcherAcknowledgement) AcknowledgeDispatcher();
                        else ActivateControl(physicalAction);
                    }
                    else AcknowledgeDispatcher();
                });
            }
        ...
        {
            // The large red safety target is the accessible input surface; keep the matching
            // physical 3D button in sync so it never feels like a decorative duplicate.
            interior3D?.PressControl(CabControlAction.DispatcherRadio);
        ...
        {
            // Keep the developing physical console visually coupled to its proven large touch
            // target. This does not alter keyboard/touch ownership or accessibility timing.
            interior3D?.PressControl(action);
            switch (action)
            {
                case CabControlAction.Horn:
                    services.Audio.Play(SoundCue.Horn);
                    world?.TryReactToScenery("meeting train", CabInteractionReaction.ReplyLight, 3f);
                    world?.TryReactToScenery("worker", CabInteractionReaction.Wave, 3f);
        ...
        {
            int onboard = passengerReport != null ? passengerReport.Onboard : 0;
            int capacity = passengerReport != null ? passengerReport.Capacity : 76;
            speedDisplay.SetText("{0:0} км/ч\nТяга {1:0}%\n<size=62%>ПАСС {2}/{3}</size>",
                motion.SpeedKph, motion.Throttle01 * 100f, onboard, capacity);
            UpdateRadioPlayer();
        }

        private void UpdateLighting(float deltaTime)
        {
            bool tunnelAutoLight = world != null && world.TunnelBlend > 0.10f;
            bool effectiveHeadlights = headlights || tunnelAutoLight;
            bool effectiveCabinLight = cabinLight || tunnelAutoLight;
            float headlightTarget = effectiveHeadlights
                ? Mathf.Lerp(services.CabRide.HeadlightLandscapeAlpha, services.CabRide.HeadlightTunnelAlpha, world.TunnelBlend)
                : 0f;
            float cabinTarget = effectiveCabinLight ? services.CabRide.CabinLightAlpha : 0f;
        ...
        public static float HybridPhotoOverlayOpacity(float configuredOpacity)
        {
            return Mathf.Clamp(configuredOpacity, 0f, 0.12f);
        }
